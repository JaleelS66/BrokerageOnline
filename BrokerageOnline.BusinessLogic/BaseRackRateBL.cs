...
namespace BrokerageOnline.BusinessLogic
{
    public class BaseRackRateBL
    {
            ...
            try
            {
                ...
                if (UserId != 0)
                {
                    ...
                    string PaymentMemoInsertQuery = "INSERT INTO [PaymentMemo] " +
                                                    ...
                                                    "[SlabType],[SlabAmount],[SlabCondition],[Remarks],[Comments],[MemoStatus],[CreatedBy],[CreatedDate],[ModifiedBy],[ModifiedDate], [IsActive],[TransactionTypeOthers],[CopiedMemoID],[SIPNotes],[IsCloseEnded],[PaymentMemoLinkId],[IsSaved]) VALUES(" +
                                                    "@BranchId, @ZoneId, @MemoTypeId, @PaymentAmount, @DateFrom, @DateTo, @ApplicableTo, @TransactionType," +
                                                    "@SlabType, @SlabAmount, @SlabCondition, @Remarks, @Comments,@MemoStatus,@CreatedBy, @CreatedDate, @ModifiedBy,@ModifiedDate,@IsActive,@TransactionTypeOthers,@CopiedMemoID,@SIPNotes,@IsCloseEnded,@PaymentMemoLinkId,@IsSaved)" +
        ...
        public string ValidateAvailableSchemeTieup(string SchemeID, string ARN, string DistributorCategory, string DateFrom, string DateTo)
        {
            ...
            string differentClawback = "Selected schemes have different Clawback, hence cannot raise tie up. Raise separate tie up memo.";
            ...
            try
            {
                ...
                if (UserId != 0)
                {
                    ...
                    string PaymentMemoInsertQuery = "INSERT INTO [RegeneratePaymentMemo] " +
                                                    "([BranchId],[ZoneId],[MemoTypeId],[PaymentAmount],[DateFrom],[DateTo],[ApplicableTo],[TransactionType]," +
                                                    "[SlabType],[SlabAmount],[SlabCondition],[Remarks],[Comments],[MemoStatus],[CreatedBy],[CreatedDate],[ModifiedBy],[ModifiedDate], [IsActive],[TransactionTypeOthers],[CopiedMemoID],[SIPNotes],[IsCloseEnded],[PaymentMemoLinkId]) VALUES(" +
                                                    "@BranchId, @ZoneId, @MemoTypeId, @PaymentAmount, @DateFrom, @DateTo, @ApplicableTo, @TransactionType," +
                                                    "@SlabType, @SlabAmount, @SlabCondition, @Remarks, @Comments,@MemoStatus,@CreatedBy, @CreatedDate, @ModifiedBy,@ModifiedDate,@IsActive,@TransactionTypeOthers,@CopiedMemoID,@SIPNotes,@IsCloseEnded,@PaymentMemoLinkId)" +
                                                    "  SELECT SCOPE_IDENTITY()";
                    Int64 PaymentMemoID = dal.SavePaymentMemo(Memo, UserId, PaymentMemoInsertQuery);
